...
function iku_cast_veils(trigger)
	...
	local inspect = require 'inspect'
	assert(io.open("C:\\Users\\Nick\\Downloads\\debugstuff.txt", 'w')):write(inspect(trigger.ability))
	if(iku_charge_pct==nil) then
		iku_charge_pct=0
	end
	if(iku_charge_pct>=0) then
		print("Enough charge.")
		trigger.caster:AddNewModifier(trigger.caster, trigger.ability, "iku_veils_buff", trigger.ability)
	else
